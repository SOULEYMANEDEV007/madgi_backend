<?php

namespace App\Http\Controllers;

use App\Exports\InternExport;
use App\Http\Requests\CreateInternRequest;
use App\Http\Requests\UpdateInternRequest;
use App\Http\Controllers\AppBaseController;
use App\Models\Admin;
use App\Models\Departement;
use App\Models\Emargement;
use App\Models\Grade;
use App\Models\Parametre;
use App\Models\Service;
use App\Models\Site;
use App\Models\User;
use App\Repositories\InternRepository;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Flash;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Support\Facades\Hash;

class InternController extends AppBaseController
{
    /** @var InternRepository $internRepository*/
    private $internRepository;

    private $user;

    public function __construct(InternRepository $internRepo)
    {
        $this->internRepository = $internRepo;
        $this->user = auth()->guard(Admin::$guard);
    }

    /**
     * Display a listing of the Intern.
     */
    public function index(Request $request)
    {
        if($this->user->check()) {
            if($this->user->user()->role->id == 1) {
                $interns = $this->internRepository->where('type', 2)->where('statut', 1)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->internRepository->where('type', 2)->where('statut', 1)->count();
                $totalLock = $this->internRepository->where('type', 2)->where('statut', 0)->count();
            }
            else {
                $interns = $this->internRepository->where('type', 2)->where('statut', 1)->where('department_id', $this->user->user()->depart->id)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->internRepository->where('type', 2)->where('department_id', $this->user->user()->depart->id)->where('statut', 1)->count();
                $totalLock = $this->internRepository->where('type', 2)->where('department_id', $this->user->user()->depart->id)->where('statut', 0)->count();
            }
        }

        $services = Service::orderBy('name','ASC')->get();
        $grades = Grade::orderBy('name','ASC')->get();
        $sites = Site::orderBy('name','ASC')->get();
        $departements = Departement::orderBy('name','ASC')->get();
        $type = 'enable';

        return view('interns.index', compact('interns', 'sites','services','departements','grades', 'total', 'totalLock', 'type'));
    }

    public function userDisable(Request $request)
    {
        if($this->user->check()) {
            if($this->user->user()->role->id == 1) {
                $interns = $this->internRepository->where('type', 2)->where('statut', 0)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->internRepository->where('type', 2)->where('statut', 1)->count();
                $totalLock = $this->internRepository->where('type', 2)->where('statut', 0)->count();
            }
            else {
                $interns = $this->internRepository->where('type', 2)->where('statut', 0)->where('department_id', $this->user->user()->depart->id)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->internRepository->where('type', 2)->where('statut', 1)->where('department_id', $this->user->user()->depart->id)->count();
                $totalLock = $this->internRepository->where('type', 2)->where('statut', 0)->where('department_id', $this->user->user()->depart->id)->count();
            }
        }

        $services = Service::orderBy('name','ASC')->get();
        $grades = Grade::orderBy('name','ASC')->get();
        $sites = Site::orderBy('name','ASC')->get();
        $departements = Departement::orderBy('name','ASC')->get();
        $type = 'disable';

        return view('interns.index', compact('interns', 'sites','services','departements','grades', 'total', 'totalLock', 'type'));
    }

    /**
     * Show the form for creating a new Intern.
     */
    public function create()
    {
        return view('interns.create');
    }

    /**
     * Store a newly created Intern in storage.
     */
    public function store(CreateInternRequest $request)
    {
        $input = $request->all();

        $input['type'] = 2;
        $input['mat_without_space'] = str_replace(' ', '', $input['matricule']);
        $input['password'] = Hash::make('password');

        if(isset($_FILES['pictures']['name']) && $_FILES['pictures']['name'] != '') {
            $files = $request->file('pictures');
            $destinationPath = public_path('images/');
            $profileImage = date("YmdHis") . "." . $files->getClientOriginalExtension();
            $files->move($destinationPath, $profileImage);
            $input['photo'] = "/images/$profileImage";
        }

        $intern = $this->internRepository->create($input);

        Flash::success('Intern saved successfully.');

        return redirect(getGuardedRoute('interns.index'));
    }

    /**
     * Display the specified Intern.
     */
    public function show($id, Request $request)
    {
        $intern = $this->internRepository->find($id);

        if ($request->has('start_date') && $request->has('end_date')) {
            $start_date = Carbon::parse($request->start_date);
            $end_date = Carbon::parse($request->end_date);
            $startOfWeek = $start_date->copy()->startOfWeek(Carbon::MONDAY);
            $endOfWeek = $end_date->copy()->endOfWeek(Carbon::MONDAY);
        }
        else {
            $start_date = $end_date = Carbon::now();
            $startOfWeek = $start_date->copy()->startOfWeek(Carbon::MONDAY);
            $endOfWeek = $end_date->copy()->endOfWeek(Carbon::MONDAY);
        }
        $clause = [$startOfWeek->format('Y-m-d'), $endOfWeek->format('Y-m-d')];
        $register = Emargement::whereUserId($id)->whereBetween('date', $clause)->count();
        $parametre = Parametre::whereSlug('heure_arrive')->first();
        $later = Emargement::whereUserId($id)->whereBetween('date', $clause)->where('heure_arrive', '>', $parametre->value)->count();

        $absences = 0;
        for ($day = $start_date->copy(); $day->lte($end_date); $day->addDay()) {
            $registration = Emargement::whereUserId($id)->where('date', $day->format('Y-m-d'))->first();
            if (!$registration) $absences++;
        }

        if (empty($intern)) {
            Flash::error('Intern not found');

            return redirect(getGuardedRoute('interns.index'));
        }

        return view('interns.show', compact('intern', 'register', 'later', 'start_date', 'end_date', 'absences'));
    }

    /**
     * Show the form for editing the specified Intern.
     */
    public function edit($id)
    {
        $intern = $this->internRepository->find($id);

        $sites = Site::orderBy('name','ASC')->get();
        $departments = Departement::orderBy('name','ASC')->get();
        $services = Service::orderBy('name','ASC')->get();

        if (empty($intern)) {
            Flash::error('Intern not found');

            return redirect(getGuardedRoute('interns.index'));
        }

        return view('interns.edit', compact('intern', 'sites', 'departments', 'services'));
    }

    /**
     * Update the specified Intern in storage.
     */
    public function update($id, UpdateInternRequest $request)
    {
        $intern = $this->internRepository->find($id);

        if (empty($intern)) {
            Flash::error('Intern not found');

            return redirect(getGuardedRoute('interns.index'));
        }

        $input = $request->all();

        if(isset($_FILES['pictures']['name']) && $_FILES['pictures']['name'] != '') {
            $files = $request->file('pictures');
            $destinationPath = public_path('images/');
            $profileImage = date("YmdHis") . "." . $files->getClientOriginalExtension();
            $files->move($destinationPath, $profileImage);
            $input['photo'] = "/images/$profileImage";
        }

        $intern = $this->internRepository->update($input, $id);

        Flash::success('Intern updated successfully.');

        return redirect(getGuardedRoute('interns.index'));
    }

    /**
     * Remove the specified Intern from storage.
     *
     * @throws \Exception
     */
    public function destroy($id)
    {
        $intern = $this->internRepository->find($id);

        if (empty($intern)) {
            Flash::error('Intern not found');

            return redirect(getGuardedRoute('interns.index'));
        }

        $this->internRepository->delete($id);

        Flash::success('Intern deleted successfully.');

        return redirect(getGuardedRoute('interns.index'));
    }

    public function search(Request $request)
    {
        $date = Carbon::now();
        $ageDate = $request->age ? $date->subYears($request->age)->format('Y') : null;
        $ancienneteDate = $request->anciennete ? $date->subYears($request->anciennete)->startOfYear()->format('Y-m-d') : null;
        $ancienneteDate1 = $request->anciennete1 ? $date->subYears($request->anciennete1)->format('Y') : null;

        $query = User::query();
        if ($request->has('nom') && $request->nom != '') $query->where('nom', 'like', '%' . $request->nom . '%');
        if ($request->has('statut') && $request->statut != '') $query->where('statut_mad', $request->statut);
        if ($request->has('contact') && $request->contact != '') $query->where('tel', 'like', '%' . $request->contact . '%');
        if ($request->has('matricule') && $request->matricule != '') $query->where('matricule', $request->matricule);
        if ($request->has('service') && $request->service != '') $query->where('service', 'like', '%' . $request->service . '%');
        if ($request->has('age') && $request->age != '') $query->orWhereYear('date_naissance', $ageDate);
        if ($request->has('grade') && $request->grade != '') foreach ($request->grade as $value) $query->where('grade', $value)->orWhere('grade', $value);
        // if ($request->has('date_entre_m') && $request->date_entre_m != '') $query->whereDate('date_entre_mad', $request->date_entre_m);
        if ($request->has('anciennete') && $request->anciennete != '') $query->where('date_entre_mad', '<=', $ancienneteDate);
        if ($request->has('anciennete1') && $request->anciennete1 != '') $query->whereYear('date_entre_mad', $ancienneteDate1);
        if ($request->has('genre') && $request->genre != '') $query->where('genre', $request->genre);
        if ($request->has('fonction') && $request->fonction != '') $query->where('fonction', 'like', '%' . $request->fonction . '%');
        if ($request->has('site') && $request->site != '') $query->where('site', 'like', '%' . $request->site . '%');
        if ($request->has('departement') && $request->departement != '') $query->where('departement', 'like', '%' . $request->departement . '%');
        $type = $request->type;
        $interns = $query->where('type', 2)
                        ->where('statut', $type == 'disable' ? 0 : 1)
                        ->orderBy('nom', 'asc')
                        ->paginate($request->paginator)
                        ->appends($request->all());

        $services = Service::orderBy('name','ASC')->get();
        $grades = Grade::orderBy('name','ASC')->get();
        $sites = Site::orderBy('name','ASC')->get();
        $departements = Departement::orderBy('name','ASC')->get();
        $total = $query->where('type', 2)->where('statut', 1)->count();
        $totalLock = $query->where('type', 2)->where('statut', 0)->count();

        return view('interns.index', compact('interns', 'sites','services','departements','grades', 'total', 'totalLock', 'type'));
    }

    public function toogle($id)
    {
        $intern = $this->internRepository->find($id);

        if (empty($intern)) {
            Flash::error('Intern not found');

            return redirect(getGuardedRoute('interns.index'));
        }

        $intern = $this->internRepository->update(['statut' => !$intern->statut], $id);

        Flash::success('Intern modifié avec succès.');

        return redirect(getGuardedRoute('interns.index'));
    }

    public function export(Request $request)
    {
        return Excel::download(new InternExport($request), 'stagiaire.xlsx');
    }
}

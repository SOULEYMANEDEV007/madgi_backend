...
{
    /** @var OfficialRepository $retirementRepository*/
    private $retirementRepository;

    private $user;

    public function __construct(OfficialRepository $retirementRepo)
    {
    ...
    public function index(Request $request)
    {
        if($this->user->check()) {
            if($this->user->user()->role->id == 1) {
                $retirements = $this->retirementRepository->where('is_salarie', 6)->where('statut', 1)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->retirementRepository->where('is_salarie', 6)->where('statut', 1)->count();
                $totalLock = $this->retirementRepository->where('is_salarie', 6)->where('statut', 0)->count();
            }
            else {
                $retirements = $this->retirementRepository->where('is_salarie', 6)->where('statut', 1)->where('department_id', $this->user->user()->depart->id)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->retirementRepository->where('is_salarie', 6)->where('department_id', $this->user->user()->depart->id)->where('statut', 1)->count();
                $totalLock = $this->retirementRepository->where('is_salarie', 6)->where('department_id', $this->user->user()->depart->id)->where('statut', 0)->count();
            }
        }
        ...
        return view('retirements.index', compact('retirements', 'sites','services','departements','grades', 'total', 'totalLock', 'type'));
    }
    ...
    public function userDisable(Request $request)
    {
        if($this->user->check()) {
            if($this->user->user()->role->id == 1) {
                $retirements = $this->retirementRepository->where('is_salarie', 6)->where('statut', 0)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->retirementRepository->where('is_salarie', 6)->where('statut', 1)->count();
                $totalLock = $this->retirementRepository->where('is_salarie', 6)->where('statut', 0)->count();
            }
            else {
                $retirements = $this->retirementRepository->where('is_salarie', 6)->where('statut', 0)->where('department_id', $this->user->user()->depart->id)->orderBy('nom', 'asc')->paginate(10);
                $total = $this->retirementRepository->where('is_salarie', 6)->where('department_id', $this->user->user()->depart->id)->where('statut', 1)->count();
                $totalLock = $this->retirementRepository->where('is_salarie', 6)->where('department_id', $this->user->user()->depart->id)->where('statut', 0)->count();
            }
        }
    ...
    public function create()
    {
        ...
        return view('retirements.create', compact('sites', 'departments', 'grades', 'services'));
    }
    ...
    public function store(CreateOfficialRequest $request)
    {
        ...
        Flash::success('retirement saved successfully.');

        return redirect(getGuardedRoute('retirements.index'));
    }
    ...
    {
        $retirement = $this->retirementRepository->find($id);

        if (empty($retirement)) {
            Flash::error('retirement not found');

            return redirect(getGuardedRoute('retirements.index'));
        }

        return view('retirements.show')->with('retirement', $retirement);
    }
    ...
    public function edit($id)
    {
        ...
        return view('retirements.edit', compact('retirement', 'sites', 'departments', 'services', 'grades'));
    }
    ...
    public function update($id, UpdateOfficialRequest $request)
    {
        ...
        $retirement = $this->retirementRepository->update($request->all(), $id);

        Flash::success('retirement updated successfully.');
    ...
    public function destroy($id)
    {
        ...
        $this->retirementRepository->delete($id);

        Flash::success('retirement deleted successfully.');
    ...
    public function search(Request $request)
    {
        ...
        $type = $request->type;
        $retirements = $query->where('is_salarie', 6)
        ...
        $sites = Site::orderBy('name','ASC')->get();
        $departements = Departement::orderBy('name','ASC')->get();
        $total = $query->where('is_salarie', 6)->where('statut', 1)->count();
        $totalLock = $query->where('is_salarie', 6)->where('statut', 0)->count();
    ...
    public function toogle($id)
    {
        ...
        $retirement = $this->retirementRepository->update(['statut' => !$retirement->statut], $id);

        Flash::success('retirement modifié avec succès.');
    ...
    {
        return Excel::download(new RetirementExport($request), 'Retraites.xlsx');
    }
}

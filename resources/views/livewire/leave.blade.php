<div>
    ...
    <div class="row justify-content-start">
        ...
        <div class="col-md-4 mb-3">
            <label for="searchUser">Nom & prénoms</label>
            <select wire:model.lazy="selectedMatricule" class="form-control form-control-sm @error('searchUser') is-invalid @enderror" name="searchUser" data-placeholder="Nom et prénoms">
                <option selected value="null" disabled>-- Choisissez un personnel --</option>
                @foreach ($users as $item)
                    <option value="{{$item->matricule}}" data-image="{{$item->photo != null ? $item->photo : asset('/images/user.png')}}">{{$item->nom}}</option>
                @endforeach
            </select>
            @error('searchUser')

...
@section('content')
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="d-flex">
                        Stagiaires
                        <form action="{{ getGuardedRoute('interns-export') }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-primary btn-sm  ml-2">
                                <i class="fa fa-download"></i> Exporter
                            </button>

                            <a href="{{getGuardedRoute('interns.create')}}" class="btn btn-primary btn-sm">
                                Ajouter
                            </a>
    ...
    <div class="content px-3">
        ...
        <div class="card text-left">
            <div class="card-body">
                <form action="{{ getGuardedRoute('interns.search', ['type' => $type]) }}" method="POST">
                    @csrf
                    <div class="row justify-content-start">
                            ...
                            <div class="row justify-content-end">
                                <div class="d-flex">
                                    <h6 class="text-primary text-bold">
                                        <a href="{{getGuardedRoute('interns.index')}}" class="text-primary">Total (pers). actif: {{$total}}</a>
                                    </h6>
                                    <h6 class="text-bold ml-5">
                                        <a href="{{getGuardedRoute('interns.userDisable')}}" class="text-danger">Total (pers). inactif: {{$totalLock}}</a>
                                    </h6>
                                </div>
                            </div>
                        </div>

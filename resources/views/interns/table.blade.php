<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('interns.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="interns-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
                ...
                <th class="text-sm">Type de stage</th>
                <th class="text-sm">Situation de stage</th>
            ...
            @foreach($interns as $key => $item)
                ...
                <tr>
                    ...
                    <td>
                        <span class="text-sm">{{$item->situation_stage}}</span>
                    </td>
                    <td>
                        <span class="text-sm">{{$item->tel}}</span>
                    </td>
                    <td>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::INTERN['READ'])
                                <a href="{{ getGuardedRoute('interns.show', [$item->id]) }}"
                                class='btn btn-primary btn-sm'>
                                    <i class="far fa-eye"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::INTERN['UPDATE'])
                                <a href="{{ getGuardedRoute('interns.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::INTERN['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'interns.toogle', $item->id], 'method' => 'put']) !!}
                                    {!! Form::button('<i class="fa fa-' . ($item->statut ? 'lock' : 'unlock') . '"></i>', ['type' => 'submit', 'class' => 'btn btn-warning btn-sm', 'onclick' => "return confirm($item->statut ? 'Voulez-vous désactivé ce stagiaire ?' : 'Voulez-vous activé ce stagiaire ?')"]) !!}
                                {!! Form::close() !!}
                            @endcan

                            @can(App\Models\Permission::INTERN['DELETE'])
                                {!! Form::open(['route' => [$guard . 'interns.destroy', $item->id], 'method' => 'delete']) !!}
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $interns])
        </div>
    </div>
</div>

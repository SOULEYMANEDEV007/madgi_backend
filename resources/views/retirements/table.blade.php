<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('retirements.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="retirements-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
            ...
            <tbody>
            @foreach($retirements as $key => $item)
                <tr>
                    <td>{{$key + 1}}</td>
                    <td>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::EMPLOYEE['READ'])
                                <a href="{{ getGuardedRoute('retirements.show', [$item->id]) }}"
                            ...
                            @can(App\Models\Permission::EMPLOYEE['UPDATE'])
                                <a href="{{ getGuardedRoute('retirements.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::EMPLOYEE['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'retirements.toogle', $item->id], 'method' => 'put']) !!}
                            ...
                            @can(App\Models\Permission::EMPLOYEE['DELETE'])
                                {!! Form::open(['route' => [$guard . 'retirements.destroy', $item->id], 'method' => 'delete']) !!}
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $retirements])
        </div>
    </div>
</div>

<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('fixed.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="fixed-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::FIXED['READ'])
                                <a href="{{ getGuardedRoute('fixed.show', [$item->id]) }}"
                                class='btn btn-primary btn-sm'>
                                    <i class="far fa-eye"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::FIXED['UPDATE'])
                                <a href="{{ getGuardedRoute('fixed.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::FIXED['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'fixed.toogle', $item->id], 'method' => 'put']) !!}
                            ...
                            @can(App\Models\Permission::FIXED['DELETE'])
                                {!! Form::open(['route' => [$guard . 'fixed.destroy', $item->id], 'method' => 'delete']) !!}
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $fixeds])
        </div>
    </div>
</div>

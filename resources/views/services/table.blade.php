<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('services.search') }}" method="GET">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="services-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
                ...
                @foreach($services as $key => $item)
                    <tr>
                        ...
                        <td  style="width: 120px">
                            <div x-data="{
                                show($item, $route) {
                                    $('#update-services-modal').modal('show');
                                    $('.services-form-action').attr('action', $route);
                                    $('.name-input-field').val($item.name);
                                },
                            }" class='btn-group'>
                                @can(App\Models\Permission::GRADE['UPDATE'])
                                    <a x-on:click="show({{$item}}, '{{getGuardedRoute('services.update', $item->id)}}')" href="javascript:;" class='btn btn-warning btn-sm'>
                                        <i class="far fa-edit"></i>
                                    </a>
                                @endcan

                                @can(App\Models\Permission::GRADE['DELETE'])
                                    {!! Form::open(['route' => [$guard . 'services.destroy', $item->id], 'method' => 'delete']) !!}
...
@include('services.modals.update')

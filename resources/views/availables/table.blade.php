<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('availables.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="availables-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
            ...
            <tbody>
            @foreach($availables as $key => $item)
                <tr>
                    <td>{{$key + 1}}</td>
                    <td>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::AVAILABLE['READ'])
                                <a href="{{ getGuardedRoute('availables.show', [$item->id]) }}"
                                class='btn btn-primary btn-sm'>
                                    <i class="far fa-eye"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::AVAILABLE['UPDATE'])
                                <a href="{{ getGuardedRoute('availables.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::AVAILABLE['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'availables.toogle', $item->id], 'method' => 'put']) !!}
                                    {!! Form::button('<i class="fa fa-' . ($item->statut ? 'lock' : 'unlock') . '"></i>', ['type' => 'submit', 'class' => 'btn btn-warning btn-sm', 'onclick' => "return confirm($item->statut ? 'Voulez-vous désactivé ce utilisateur ?' : 'Voulez-vous activé ce utilisateur ?')"]) !!}
                                {!! Form::close() !!}
                            @endcan

                            @can(App\Models\Permission::AVAILABLE['DELETE'])
                                {!! Form::open(['route' => [$guard . 'availables.destroy', $item->id], 'method' => 'delete']) !!}
    ...
    <div class="card-footer clearfix">
        <div class="float-right">
            @include('adminlte-templates::common.paginate', ['records' => $availables])
        </div>
    </div>
</div>

...
@section('content')
                ...
                <div class="col-sm-6">
                    <h1 class="d-flex">
                        ...
                        @can(App\Models\Permission::WORKER['EXPORT'])
                            <form action="{{ getGuardedRoute('workers-export') }}" method="POST">
                                @csrf
                                ...
                                <div class="row justify-content-start">
                                    ...
                                    <input class="form-control form-control-sm" name="is_medical" value="{{ request('is_medical') }}" type="hidden">
                                </div>
                            </form>
                        @endcan
                    </h1>
    ...
    <div class="content px-3">
        ...
        <div class="card text-left">
            <div class="card-body">
                <form action="{{ getGuardedRoute('workers.search', ['type' => $type]) }}" method="POST">
                    @csrf
                    <div class="row justify-content-start">
                        ...
                        <div class="col-md-3 form-group">
                            <select class="form-control form-control-sm" name="is_medical" aria-label=".form-select-lg example">
                                <option value="null" selected disabled>Sélectionnez la catégorie</option>
                                <option value="1" @if(request('is_medical') == 1) selected @endif>Personnel médical</option>
                                <option value="0" @if(request('is_medical') == 1) selected @endif>Personnel non médical</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            ...
                            <div class="row justify-content-end">
                                <div class="d-flex">
                                    <h6 class="text-primary text-bold">
                                        <a href="{{getGuardedRoute('workers.index')}}" class="text-primary">Total (pers). actif: {{$total}}</a>
                                    </h6>
                                    <h6 class="text-bold ml-5">
                                        <a href="{{getGuardedRoute('workers.userDisable')}}" class="text-danger">Total (pers). inactif: {{$totalLock}}</a>
                                    </h6>
                                </div>
                            </div>
                        </div>

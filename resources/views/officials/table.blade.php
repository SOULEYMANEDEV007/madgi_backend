<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('officials.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="officials-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::OFFICIAL['READ'])
                                <a href="{{ getGuardedRoute('officials.show', [$item->id]) }}"
                                class='btn btn-primary btn-sm'>
                                    <i class="far fa-eye"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::OFFICIAL['UPDATE'])
                                <a href="{{ getGuardedRoute('officials.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::OFFICIAL['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'officials.toogle', $item->id], 'method' => 'put']) !!}
                            ...
                            @can(App\Models\Permission::OFFICIAL['DELETE'])
                                {!! Form::open(['route' => [$guard . 'officials.destroy', $item->id], 'method' => 'delete']) !!}

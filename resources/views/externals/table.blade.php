<div class="card-body">
    <form id="paginationForm" action="{{ getGuardedRoute('externals.search') }}" method="POST">
        @csrf
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped text-center" id="externals-table">
            <thead>
            <tr>
                <th class="text-sm">#</th>
                    ...
                    <td  style="width: 120px">
                        <div class='btn-group'>
                            @can(App\Models\Permission::EXTERNAL['READ'])
                                <a href="{{ getGuardedRoute('externals.show', [$item->id]) }}"
                                class='btn btn-primary btn-sm'>
                                    <i class="far fa-eye"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::EXTERNAL['UPDATE'])
                                <a href="{{ getGuardedRoute('externals.edit', [$item->id]) }}"  class='btn btn-info btn-sm'>
                                    <i class="far fa-edit"></i>
                                </a>
                            @endcan

                            @can(App\Models\Permission::EXTERNAL['DISABLE'])
                                {!! Form::open(['route' => [$guard . 'externals.toogle', $item->id], 'method' => 'put']) !!}
                            ...
                            @can(App\Models\Permission::EXTERNAL['DELETE'])
                                {!! Form::open(['route' => [$guard . 'externals.destroy', $item->id], 'method' => 'delete']) !!}
